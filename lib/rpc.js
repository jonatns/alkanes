...
class AlkanesRpc extends base_rpc_1.BaseRpc {
    ...
    async listdeployedalkanes(blockTag = "latest") {
        const payload = (0, outpoint_1.encodeBlockHeightInput)(0);
        const response = await this._call({
            method: "listdeployedalkanes",
            input: payload,
        }, blockTag);
        // Parse like other responses
        return alkanes_1.alkanes.DeployedAlkanesResponse.deserializeBinary(Buffer.from(response.slice(2), "hex"));
    }
